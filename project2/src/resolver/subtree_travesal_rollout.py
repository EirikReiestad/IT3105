...
class SubtreeTraversalRollout:
    ...
    def subtree_traversal_rollout(
        ...
        if SubtreeTraversalRollout.showdown_state(state):
            p_values = (
                SubtreeTraversalRollout.utility_matrix(state, p_range, o_range)
                * o_range.T
            )
            o_values = -p_values * SubtreeTraversalRollout.utility_matrix(
                state, o_range, p_range
            )
            ...
            for action in state.get_legal_actions():
                ...
                p_range[action] = Resolver.bayesianRangeUpdate(
                    p_range, state, action, o_range
                )
                # TODO: This should be index by action (see pseudocode)
                o_range[action] = o_range
                state = state.get_next_state(action)
                p_values[action], o_values[action] = (
                ...
                for pair in hole_pairs:
                    p_values[pair] += (
                        Resolver.strategy(pair, action) * p_values[action][pair]
                    )
        else:
            p_values = np.zeros((0, 0))
            o_values = np.zeros((0, 0))
